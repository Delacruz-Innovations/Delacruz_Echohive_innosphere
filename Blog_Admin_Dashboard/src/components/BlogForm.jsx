...
const BlogForm = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        type: 'Article', // For EchoHive
        subtitle: '', // For EchoHive
        hasRegister: false, // For EchoHive
        tags: [],
        relatedPosts: [],
        featured: false,
        status: 'draft',
    ...
    const addAuthor = () => {
        setFormData(prev => ({
            ...prev,
            authors: [...prev.authors, { name: '', bio: '', image: '' }]
        }));
    };
    ...
    const handlePointChange = (sectionIndex, pointIndex, field, value) => {
        const newSections = [...formData.content.sections];
        newSections[sectionIndex].points[pointIndex][field] = value;
        setFormData(prev => ({
            ...prev,
            content: { ...prev.content, sections: newSections }
        }));
    ...
    const handleSectionImageUpload = async (e, index) => {
        ...
        try {
            // Since we don't have a storage service yet, we'll convert to Base64 for now
    ...
    const handleSave = async (status = 'draft') => {
        ...
        try {
            ...
            showNotification(`${isDelacruz ? 'Innovation' : isEchoHive ? 'News' : 'Blog'} saved successfully!`, 'success');
            navigate(`/${orgId}/dashboard`);
        } catch (error) {
            showNotification(`Error saving ${isDelacruz ? 'innovation' : isEchoHive ? 'news' : 'blog'}: ` + error.message, 'error');
        } finally {
            setIsActionLoading(false);
        }
    };
    ...
    return (
        <>
                                                ...
                                                <article className="lg:col-span-9">
                                                    ...
                                                    <header className="mb-16 border-b border-white/10 pb-16">
                                                        <div className="flex items-center gap-4 mb-8">
                                                            <span className="px-4 py-1.5 bg-[#6b9dc7] text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-sm">
                                                                {formData.category || "General"}
                                                            </span>
                                                            {formData.type && (
                                                                <span className="px-4 py-1.5 bg-white/10 text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-sm border border-white/10">
                                                                    {formData.type}
                                                                </span>
                                                            )}
                                                            <span className="text-gray-500 text-[10px] font-bold uppercase tracking-[0.2em]">
                                                                {formData.date || "DRAFT ISSUE"}
                                                            </span>
                                                        </div>
                                                        <h1 className="text-5xl md:text-7xl font-bold text-white mb-8 tracking-tighter leading-[0.9] font-outfit uppercase text-left italic">
                                                            {formData.title || "Untitled Report"}
                                                        </h1>
                                                        {formData.subtitle && (
                                                            <p className="text-xl md:text-2xl text-[#6b9dc7] font-medium mb-8 font-outfit text-left uppercase tracking-tight">
                                                                {formData.subtitle}
                                                            </p>
                                                        )}
                                                        <div className="flex flex-wrap gap-3">
                                                            {formData.tags.map(tag => (
                                                                <span key={tag} className="text-[10px] font-bold text-[#6b9dc7] uppercase tracking-widest border border-[#6b9dc7]/30 px-3 py-1 rounded-full">
                                                                    #{tag}
                                                                </span>
                                                            ))}
                                                        </div>
                                                    ...
                                                    <div className="space-y-24">
                                                        {formData.content.sections.map((section, index) => (
                                                            <div key={index}>
                                                                ...
                                                                <div className="prose prose-invert prose-lg max-w-none mb-8 text-left">
                                                                    <div
                                                                        className="text-gray-300 leading-relaxed space-y-6 text-lg font-inter ql-editor-preview"
                                                                ...
                                                                {section.hasPoints && section.points && section.points.length > 0 && (
                                                                    <div className="mb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                                                                        {section.points.map((point, pIdx) => (
                                                                            <div key={pIdx} className="bg-white/5 border-l-2 border-[#6b9dc7] p-6 rounded-r-xl group hover:bg-white/10 transition-all">
                                                                                <h4 className="text-white font-bold uppercase tracking-widest text-sm mb-2 group-hover:text-[#6b9dc7] transition-colors">
                                                                                    {point.title}
                                                                                </h4>
                                                                ...
                                                                {section.hasInsight && section.insight && (
                                                                    <div className="mt-10 bg-gradient-to-br from-[#6b9dc7]/20 to-blue-900/10 border border-[#6b9dc7]/30 rounded-2xl p-8 relative overflow-hidden shadow-inner">
                                                    ...
                                                    {/* Conclusion */}
                                                    {formData.content.conclusion && formData.content.conclusion.body && (
                                                        <div className="mt-24 p-10 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-sm">
                                                            <h2 className="text-2xl font-bold text-white mb-6 uppercase tracking-widest font-outfit text-left flex items-center gap-3">
                                                                <CheckCircle className="text-[#6b9dc7]" />
                                                                {formData.content.conclusion.heading || "Executive Conclusion"}
                                                            </h2>
                                                            <div
                                                                className="text-lg text-gray-300 leading-relaxed font-inter text-left ql-editor-preview"
                                                                dangerouslySetInnerHTML={{ __html: formData.content.conclusion.body }}
                                                            />
                                                        </div>
                                                    )}
                                                    ...
                                                    <div className="mt-24 pt-16 border-t border-white/10 space-y-8">
                                                        {(formData.authors || []).map((author, index) => (
                                                            <div key={index} className="flex flex-col md:flex-row items-center md:items-start gap-8 bg-white/5 p-8 rounded-2xl border border-white/10 font-inter">
                                                                {author.image ? (
                                                                    <img src={author.image} className="w-24 h-24 rounded-full object-cover border border-[#6b9dc7]/30" alt={author.name} />
                                                                ) : (
                                                                ...
                                                                <div className="text-center md:text-left">
                                                                    <h4 className="text-2xl font-bold text-white mb-4 uppercase font-outfit">
                                                                        About <span className="text-[#6b9dc7]">{author.name || "Innosphere Editorial"}</span>
                                                                    </h4>
                                                                    <p className="text-gray-400 leading-relaxed text-lg text-left">
                                                                        {author.bio || "Innosphere Consulting strategy and insight team."}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        ))}
                        ...
                        <section className="bg-white border border-gray-200 rounded-sm overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                            ...
                            <div className="p-6 space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                    ...
                                    {isEchoHive && (
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2">News Subtitle</label>
                                            <input
                                                type="text"
                                                name="subtitle"
                                                className={`w-full px-4 py-3 border border-gray-200 rounded-sm text-gray-900 font-medium focus:border-${orgTheme.focus} focus:ring-1 focus:ring-${orgTheme.focus} outline-none transition-all placeholder-gray-300`}
                                                placeholder="Enter secondary subtitle..."
                                ...
                                <div className="grid grid-cols-3 gap-6">
                                    ...
                                    {isEchoHive ? (
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2">Post Type</label>
                                            <select
                                                name="type"
                                        ...
                                        <div className="flex items-center pt-8">
                                            <label className="relative inline-flex items-center cursor-pointer">
                                                ...
                                                <span className="ml-3 text-[10px] font-bold text-gray-900 uppercase tracking-widest">Mark as Featured</span>
                                            </label>
                                        </div>
                                    )}
                                </div>
                                        ...
                                        <div className="flex items-center pt-8">
                                            <label className="relative inline-flex items-center cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    name="hasRegister"
                                                    checked={formData.hasRegister}
                                                    onChange={handleInputChange}
                                                    className="sr-only peer"
                                                />
                                                <div className={`w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-${orgTheme.color}`}></div>
                                                <span className="ml-3 text-[10px] font-bold text-gray-900 uppercase tracking-widest">Enable Registration Link</span>
                                            </label>
                                        </div>
                                    )}
                                </div>
                        ...
                        <section className="bg-white border border-gray-200 rounded-sm overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                            ...
                            <div className="p-6 space-y-10">
                                        ...
                                        >
                                            ...
                                            <div className="space-y-8">
                                                ...
                                                <div className="space-y-4">
                                                    <div className="flex items-center justify-between p-4 bg-gray-50/50 border border-gray-100 rounded-sm shadow-sm">
                                                        <div className="flex items-center gap-3">
                                                            <Plus className="h-4 w-4 text-gray-400" />
                                                            <span className="text-[10px] font-bold text-gray-900 uppercase tracking-widest">Structured Points / Key Takeaways</span>
                                                        </div>
                                                        <label className="relative inline-flex items-center cursor-pointer">
                                                            <input type="checkbox" checked={section.hasPoints || false} onChange={(e) => handleSectionChange(index, 'hasPoints', e.target.checked)} className="sr-only peer" />
                                                            <div className={`w-9 h-5 bg-gray-200 peer - focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-${orgTheme.color}`}></div>
                                                        </label>
                                                    </div>
                                                    {section.hasPoints && (
                                                        <div className="space-y-4 p-4 border border-gray-100 bg-gray-50/10 rounded-sm">
                                                            {section.points?.map((point, pIndex) => (
                                                                <div key={pIndex} className="flex gap-4 items-start bg-white p-3 border border-gray-100 rounded-sm shadow-sm animate-in fade-in slide-in-from-left-2 duration-200">
                                                                    <div className="flex-1 space-y-3">
                                                                        <input
                                                                            type="text"
                                                                            placeholder="POINT TITLE / HEADING..."
                                                                            className={`w-full px-3 py-2 border border-gray-100 rounded-sm text-[10px] font-bold uppercase tracking-widest focus:border-${orgTheme.focus} outline-none transition-all`}
                                                                            value={point.title}
                                                                            onChange={(e) => handlePointChange(index, pIndex, 'title', e.target.value)}
                                                                        />
                                                                        <textarea
                                                                            placeholder="POINT DESCRIPTION / DETAIL..."
                                                                            className={`w-full px-3 py-2 border border-gray-100 rounded-sm text-[10px] font-medium uppercase tracking-tighter focus:border-${orgTheme.focus} outline-none resize-none h-16 transition-all`}
                                                                            value={point.description}
                                                                            onChange={(e) => handlePointChange(index, pIndex, 'description', e.target.value)}
                                                                        />
                                                                    </div>
                                                                    <button
                                                                        type="button"
                                                            ...
                                                            <button
                                                                ...
                                                                className={`w-full py-2 border border-dashed border-gray-200 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] hover:bg-gray-50 hover:border-${orgTheme.focus}/30 hover:text-${orgTheme.color} transition-all rounded-sm`}
                                                            >
                                                                + Append Structured Point
                                                            </button>
                                                        </div>
                                                    )}
                                                </div>
                                ...
                                <div className="space-y-4 pt-6 border-t border-gray-100">
                                    <div className="flex items-center justify-between p-4 bg-gray-900 text-white rounded-sm shadow-md">
                                        <div className="flex items-center gap-3">
                                            <CheckCircle className="h-4 w-4" />
                                            <span className="text-[10px] font-bold uppercase tracking-widest">Strategic Conclusion / Summary</span>
                                        </div>
                                        <div className="text-[8px] font-bold uppercase opacity-50 tracking-widest">Final Articles Conclusion</div>
                                    </div>
                                    <div className="p-6 bg-white border border-gray-200 rounded-sm space-y-6">
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2">Conclusion Heading</label>
                                            <input
                                                type="text"
                                                placeholder="ENTER CONCLUSION TITLE..."
                                                className={`w-full px-4 py-3 border border-gray-200 rounded-sm text-sm font-bold focus:border-${orgTheme.focus} outline-none uppercase placeholder-gray-200 transition-all`}
                                                value={formData.content.conclusion?.heading || ''}
                                                onChange={(e) => setFormData(prev => ({
                                                    ...prev,
                                                    content: {
                                                        ...prev.content,
                                                        conclusion: { ...prev.content.conclusion, heading: e.target.value }
                                                    }
                                                }))}
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2">Final Summary Manuscript</label>
                                            <div className="quill-wrapper border border-gray-200 rounded-sm overflow-hidden">
                                                <ReactQuill
                                                    theme="snow"
                                                    value={formData.content.conclusion?.body || ''}
                                                    onChange={(content) => setFormData(prev => ({
                                                        ...prev,
                                                        content: {
                                                            ...prev.content,
                                                            conclusion: { ...prev.content.conclusion, body: content }
                                                        }
                                                    }))}
                                                    modules={quillModules}
                                                    formats={quillFormats}
                    ...
                    < div className="space-y-10" >
                        ...
                        <section className="bg-white border border-gray-200 rounded-sm overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                            ...
                            <div className="p-6 space-y-8">
                                {formData.authors.map((author, index) => (
                                    <div key={index} className="space-y-4 p-4 bg-gray-50/50 border border-gray-100 rounded-sm relative group">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">AUTHOR #{index + 1}</span>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2">Profile Image URL</label>
                                                <input
                                                    type="text"
                                                    placeholder="HTTPS://..."
                                                    className={`w-full px-4 py-3 border border-gray-200 rounded-sm text-xs font-mono focus:border-${orgTheme.focus} outline-none uppercase placeholder-gray-300 transition-all`}
                                                    value={author.image}
                                                    onChange={(e) => handleAuthorChange(index, 'image', e.target.value)}
                                                />
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <textarea
                                                ...
                                                className={`w-full px-4 py-3 border border-gray-200 rounded-sm h-24 text-xs font-medium focus:border-${orgTheme.focus} outline-none uppercase resize-none transition-all leading-relaxed`}
                                                value={author.bio}
            ...
            {isActionLoading && <LoadingOverlay message="Synchronizing and securing document content..." />}
        </>
    );
};
